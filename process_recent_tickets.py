...
# (Reusing exact same HTML/Text as process_personal_mail_tickets.py)
...
def main():
    print("STARTING RECENT REGISTRATION TICKET PROCESSING...")
    headers = get_supabase_headers()
    
    # 1. Fetch pending records
    # Limit to 1000 to be safe, filter OUT 'Invalid' status
    url = f"{SUPABASE_URL}/rest/v1/registeration_recent?ticket_sent=is.false&payment_status=neq.Invalid&limit=1000"
    r = requests.get(url, headers=headers)
    if r.status_code != 200:
        sys.exit(f"Error fetching pending: {r.text}")
        
    pending = r.json()
    print(f"Found {len(pending)} pending records (excluding Invalid).")
    ...
    for i, record in enumerate(pending):
        ...
        if not reg_id:
            ...
            if existing:
                ...
                new_reg = {
                    ...
                    "payment_status": "paid" # Assume paid if imported and not Invalid
                } 
                ...
                if r.status_code != 201:
                    ...
                    # If failed, maybe try to fetch again? 
            ...
            requests.patch(f"{SUPABASE_URL}/rest/v1/registeration_recent?id=eq.{record['id']}", 
        ...
        # 30 second timeout? No, let's keep it safe.
        ...
        if ok:
            ...
            requests.post(f"{SUPABASE_URL}/rest/v1/ticket_email_events", headers=headers, json={
                "registration_id": reg_id, "ticket_id": ticket_id, "to_email": email, "status": "sent", "source": "recent_import_script"
            })
        else:
            print(f"  ❌ Email FAILED: {err}")
            requests.post(f"{SUPABASE_URL}/rest/v1/ticket_email_events", headers=headers, json={
                "registration_id": reg_id, "ticket_id": ticket_id, "to_email": email, "status": "failed", "error_text": str(err), "source": "recent_import_script"
            })
