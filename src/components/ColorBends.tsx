...
type ColorBendsProps = {
  ...
  /**
   * Cap renderer pixel ratio (helps prevent WebGL context loss on mobile).
   * Defaults to min(devicePixelRatio, 1.5)
   */
...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    // Avoid a noisy (and sometimes fatal) dev-time error:
    // three.js may return `null` for shader/program logs on some GPUs/contexts,
    // but still calls `.trim()` internally when `checkShaderErrors` is true.
    // Disabling it prevents "Cannot read properties of null (reading 'trim')".
    ...
    const handleContextLost = (e: Event) => {
      // Prevent browser default behavior (which may attempt to restore automatically)
      // and stop our render loop to avoid thrashing.
      if ((e as any).preventDefault) (e as any).preventDefault();
      console.warn('[ColorBends] WebGL context lost');
      contextLostRef.current = true;
      if (rafRef.current !== null) cancelAnimationFrame(rafRef.current);
      rafRef.current = null;
    };

    // Note: restored handling is intentionally minimal; the most reliable recovery
    // is a remount, but at least we stop spamming frames once lost.
  ...
  useEffect(() => {
    ...
    target.addEventListener('pointermove', handlePointerMove as any, { passive: true } as any);
    return () => {
